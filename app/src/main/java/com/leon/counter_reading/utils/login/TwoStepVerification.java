...
public class TwoStepVerification {
    public static void insertPersonalCode(Context context) {
        ...
        lovelyTextInputDialog.setTopColorRes(R.color.yellow).setTopTitleColorRes(android.R.color.white)
                ...
                .setCancelable(false).setInputFilter(R.string.error_empty, text ->
                    ...
                    int personalCode = getDigits(lovelyTextInputDialog.getEditTextNumber().getText().toString());
    ...
    public static void showPersonalCode(Activity activity) {
        if (getApplicationComponent().SharedPreferenceModel().getIntNullData(PERSONAL_CODE.getValue()) > 0) {
            ...
            String verificationCode = String.valueOf(getApplicationComponent().SharedPreferenceModel().getIntData(PERSONAL_CODE.getValue()) + 1313 * calendarTool.getIranianMonth() * calendarTool.getIranianDay());
            CustomDialogModel dialog = new CustomDialogModel(Green, activity, verificationCode,
                    R.string.verification_code, R.string.dear_user, R.string.accepted);
