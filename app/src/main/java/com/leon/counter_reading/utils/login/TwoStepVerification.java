...
public class TwoStepVerification {
    ...
    public static void showPersonalCode(Activity activity) {
        if (getApplicationComponent().SharedPreferenceModel().getIntNullData(PERSONAL_CODE.getValue()) > 0) {
            ...
            final String verificationCode = String.valueOf(getApplicationComponent()
                    .SharedPreferenceModel().getIntData(PERSONAL_CODE.getValue()) + 1313 *
                    calendarTool.getIranianMonth() * calendarTool.getIranianDay());
            final CustomDialogModel dialog = new CustomDialogModel(Green, activity, verificationCode,
            ...
            dialog.getLovelyStandardDialog().getMessageView()
